            ...
            for iy in range(num_step_y):
                ...
                for iz in range(num_step_z):
                    ...
                    assert len(args.OutputCh) // 2 <= len(tmp_out), (
                        "the parameter OutputCh is not compatible with the number of output tensors"
                    )
                    ...
                    for ch_idx in range(len(args.OutputCh) // 2):
                        ...
                        ] = out_nda[:, :, :, args.OutputCh[ch_idx * 2 + 1]]
...
def save_checkpoint(state, checkpoint_dir, logger=None):
    ...
    file_path = (
        checkpoint_dir + os.sep + "checkpoint_epoch_" + str(state["epoch"]) + ".pytorch"
    )
...
def build_model(config):
    ...
    assert name in SUPPORTED_MODELS, (
        f"Invalid model: {name}. Supported models: {SUPPORTED_MODELS}"
    )
    ...
    if name == "unet_xy":
        ...
        model = DNN(
            config["nchannel"], config["nclass"], model_config.get("zoom_ratio", 3)
        )
