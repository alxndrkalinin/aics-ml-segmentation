...
def load_tiff_as_czyx(path):
    """Load a TIFF file and coerce to CZYX layout."""
    img = np.squeeze(tifffile.imread(path))
    if img.ndim == 3:
        img = np.expand_dims(img, axis=0)  # ZYX -> CZYX
    elif img.ndim == 4 and img.shape[0] > img.shape[1]:
        img = np.transpose(img, (1, 0, 2, 3))  # ensure C first
    return img
...
def load_single_image(args, fn, time_flag=False):
    ...
    if time_flag:
        ...
        if img_raw.ndim == 3:
            ...
            img_raw = np.transpose(img_raw, (1, 0, 2, 3))  # ensure C first
        if isinstance(args.InputCh, List):
            channel_list = args.InputCh
        else:
            channel_list = [args.InputCh]
        img = img_raw[channel_list]
